...
with col_gear:
    with st.popover("⚙️", help="管理系統清單"):
        ...
        c1, c2, c3, c4 = st.columns([1, 3, 5, 1])
        with c1:
            add_icon = st.text_input("", value="◆", key="add_icon", label_visibility="collapsed")
        with c2:
            add_name = st.text_input("", placeholder="系統名稱", key="add_name", label_visibility="collapsed")
        with c3:
            add_url  = st.text_input("", placeholder="https://xxx.streamlit.app", key="add_url", label_visibility="collapsed")
        with c4:
            st.markdown("<div style='height:4px'></div>", unsafe_allow_html=True)
...
if tab_labels:
    for tab, sys in zip(tabs, systems):
        with tab:
            components.iframe(sys["url"].rstrip("/") + "/?embed=true", height=860, scrolling=True)
